...
export function ProofGallery() {
  ...
  return (
    <>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 lg:pb-0">
          {filters.map((item) => (
            <button
              ...
              className={`shrink-0 rounded-full border px-4 py-3 text-xs font-semibold uppercase tracking-[0.12em] transition ${
                filter === item
                  ? "border-[#E1306C]/45 bg-[#E1306C]/10 text-[color:var(--foreground)]"
                  : "border-[color:var(--line)] text-muted hover:border-[#E1306C]/40 hover:text-[color:var(--foreground)]"
              }`}
            >
              {item}
            </button>
      ...
      {filtered.length ? (
        <motion.div layout className="proof-masonry">
          <AnimatePresence>
            {filtered.map((proof) => (
              <motion.button
                layout
              ...
              >
                ...
                <span className="absolute inset-x-0 bottom-0 flex translate-y-3 items-center justify-between p-4 opacity-0 transition group-hover:translate-y-0 group-hover:opacity-100">
                  <span>
                    <span className="block text-[10px] uppercase tracking-[0.15em] text-[#FCAF45]">{proof.category}</span>
        ...
        <div className="surface-card rounded-3xl p-12 text-center text-muted">
          No proofs match that search.
        </div>
      )}
      ...
      <AnimatePresence>
        {active && (
          <motion.div
            className="fixed inset-0 z-[110] grid place-items-center bg-black/95 p-3 backdrop-blur-xl sm:p-8"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            role="dialog"
          ...
          >
            <button
              type="button"
              className="absolute right-4 top-4 z-10 grid h-11 w-11 place-items-center rounded-full border border-white/10 bg-black/40"
              onClick={() => setActiveId(null)}
            ...
            <button
              type="button"
              className="absolute left-3 top-1/2 z-10 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/10 bg-black/50 sm:left-8"
              onClick={(event) => {
            ...
            <motion.div
              key={active.id}
              className="relative h-[92vh] w-full max-w-4xl"
            ...
            <button
              type="button"
              className="absolute right-3 top-1/2 z-10 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/10 bg-black/50 sm:right-8"
              onClick={(event) => {
                event.stopPropagation();
                navigate(1);
              }}
